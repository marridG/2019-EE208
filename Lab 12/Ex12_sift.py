...
print "Target Image: %s" % path_target
...
# for key_pt in key_pts_target:
#     cv2.circle(target_color, (key_pt[0], key_pt[1]), 10, (0, 0, 255))
# cv2.imwrite("target.png", target_color)
...
for cand_idx, img_candidate in enumerate(IMG_CANDIDATES):
    ...
    print "**********\nCandidate Image: %s" % path_candidate
    ...
    for idx, match in enumerate(matched_idx):
        ...
        cnt += 1
    print "\tMatches Drawn"
    print "Altogether: %s Matches" % cnt
    cv2.imwrite(OUT_PREFIX + "res %s_%s matches.png" % (cand_idx + 1, cnt), matched_image)
    # plt.imshow(matched_image, 'gray'), plt.xticks([]), plt.yticks([])
    # plt.show()
