...
def read_image(image):
    ...
    if "opencv" == GRAY_METHODS[GRAY_METHOD]:
        ...
        read = (red_channel + green_channel + blue_channel) / 3
    else:
        image_color = cv2.imread(image, cv2.IMREAD_COLOR)
        red_channel = image_color[:, :, 2]
        green_channel = image_color[:, :, 1]
...
def cal_gradient(image_read):
    ...
    if "Sobel" == GRADIENT_METHODS[GRADIENT_METHOD]:
        ...
        gradient_x = cv2.filter2D(image_read, -1, np.array([[-1, 1], [-1, 1]]))
        gradient_y = cv2.filter2D(image_read, -1, np.array([[1, 1], [-1, -1]]))
        gradient_x = gradient_x.astype(np.int32)
        gradient_y = gradient_y.astype(np.int32)
        gradient_magnitude = (gradient_x ** 2 + gradient_y ** 2) ** 0.5
        gradient_magnitude = cv2.convertScaleAbs(gradient_magnitude)
        cv2.imwrite('x.png', gradient_magnitude)
...
if TEST_THRESHOLDS:
    for i in range(10, 100, 10):
        ...
        for j in range(20, 31):
            ...
            cv2.imwrite(PLOT_PATH_PREFIX + "%s,%s.png" % (THRESHOLD_LOW, THRESHOLD_HIGH), res)
    ...
    image_suppressed = non_max_suppression(image_read=image_gauss,
                                           gradient_x=grad_x, gradient_y=grad_y,
                                           gradient_magnitude=grad_mag)
    cv2.imshow("a", image_suppressed)
    cv2.waitKey(0)
    # print image_suppressed
    res = double_threshold(image_suppressed, image_gray, grad_mag)
    cv2.imwrite(PLOT_PATH_PREFIX + "%s,%s.png" % (THRESHOLD_LOW, THRESHOLD_HIGH), res)

    canny = cv2.Canny(cv2.imread(image_path), THRESHOLD_LOW, THRESHOLD_HIGH)
    titles = ['gray', 'gauss', 'suppressed', 'res', 'canny']
    images = [image_gray, image_gauss, image_suppressed, res, canny]

    cv2.imwrite(PLOT_PATH_PREFIX + "%s,%s_canny.png" % (THRESHOLD_LOW, THRESHOLD_HIGH), canny)
    ...
    for i in xrange(4):
        ...
        plt.xticks([]), plt.yticks([])
    # plt.show()
    plt.savefig(PLOT_PATH_PREFIX + "%s,%s_comp.png" % (THRESHOLD_LOW, THRESHOLD_HIGH))
